...
class DnnClassifier:
    ...
    def _prepare_tensors(self, X: pd.DataFrame) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        if self.cat_cols:
            ...
            for col in self.cat_cols:
                ...
                cat_arrays.append(X[col].map(mapping).fillna(0).astype("int64").values)
    ...
    def fit(
        ...
        self.model = TabularNN(
            emb_dims, x_num.shape[1], self.hidden_units, self.dropout, self.num_layers
        )
...
def create_model(model_type: str, **kwargs) -> Any:
    ...
    models = {
        ...
        "random_forest": RandomForestClassifier(
            random_state=42, n_estimators=100, **kwargs
        ),
...
def preprocess_reweighing(
    X_train: pd.DataFrame, y_train: pd.Series, sensitive_features: pd.Series
) -> np.ndarray:
    ...
    dataset = BinaryLabelDataset(
        df=df_train, label_names=["y"], protected_attribute_names=["sensitive"]
    )

    # Apply reweighing
    reweighing = Reweighing(
        unprivileged_groups=[{"sensitive": 0}], privileged_groups=[{"sensitive": 1}]
    )
...
def apply_postprocessing(
    ...
    try:
        y_prob = postprocessor.predict_proba(
...
def bootstrap_metrics(
    ...
    for i in range(n_bootstrap):
        ...
        bootstrap_students = np.random.choice(
            unique_students, size=len(unique_students), replace=True
        )
        ...
        if y_prob is not None:
            sample_metrics["brier"] = brier_score_loss(
                y_true[bootstrap_mask], y_prob[bootstrap_mask]
            )
            sample_metrics["ece"] = expected_calibration_error(
                y_true[bootstrap_mask], y_prob[bootstrap_mask]
            )
...
def save_reliability_plot(
    y_true: np.ndarray, y_prob: np.ndarray, output_dir: Path, filename_prefix: str
) -> None:
    ...
    plt.savefig(
        output_dir / f"{filename_prefix}_reliability.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def save_confusion_matrix(
    y_true: np.ndarray, y_pred: np.ndarray, output_dir: Path, filename_prefix: str
) -> None:
    ...
    output_dir.mkdir(parents=True, exist_ok=True)
    plt.savefig(
        output_dir / f"{filename_prefix}_confusion.png", dpi=300, bbox_inches="tight"
    )
    plt.close()
...
def main():
    ...
    parser.add_argument(
        "--dataset", type=Path, required=True, help="Path to dataset parquet file"
    )
    parser.add_argument(
        "--split", type=Path, required=True, help="Path to split JSON file"
    )
    parser.add_argument(
        "--model",
