...
class InductiveConformalPredictor:
    ...
    def __init__(self, alpha: float = 0.1):
        ...
        logger.info(
            f"Initialized conformal predictor with alpha={alpha}, target coverage={self.target_coverage}"
        )
    ...
    def calibrate(self, y_cal: np.ndarray, y_prob_cal: np.ndarray) -> None:
        ...
        logger.info(
            f"Empirical coverage on calibration set: "
            f"{(self.calibration_scores <= self.threshold).mean():.4f}"
        )

    def predict_sets(
        self, y_prob_test: np.ndarray
        ...
        if empty_sets.any():
            logger.warning(
                f"Found {empty_sets.sum()} empty prediction sets, "
        ...
        return (
            ...
            set_sizes,
            np.zeros(n_test),
        )  # coverage_flags computed separately

    def evaluate_coverage(
        self, y_test: np.ndarray, prediction_sets: np.ndarray
    ) -> Dict[str, float]:
        ...
        coverage_flags = np.array(
            [prediction_sets[i, int(y_test[i])] for i in range(len(y_test))]
        )
    ...
    def evaluate_coverage_by_group(
        self, y_test: np.ndarray, prediction_sets: np.ndarray, groups: np.ndarray
    ) -> pd.DataFrame:
...
def run_conformal_prediction(
    ...
    for alpha in alphas:
        ...
        if groups_test is not None:
            group_results = cp.evaluate_coverage_by_group(
                y_test, pred_sets, groups_test
            )
        ...
        logger.info(
            f"Alpha {alpha}: Coverage = {overall_results['coverage']:.3f}, "
...
def test_conformal_prediction():
    ...
    results = run_conformal_prediction(y_cal, prob_cal, y_test, prob_test, groups_test)
    ...
    for alpha, alpha_results in results.items():
        ...
        logger.info(
            f"Test alpha {alpha}: Coverage = {coverage:.3f}, Target = {target:.3f}"
        )

        # Coverage should be close to target (within reasonable margin for finite samples)
        assert (
            abs(coverage - target) < 0.1
        ), f"Coverage too far from target for alpha {alpha}"
...
def main():
    ...
    for alpha, alpha_results in results.items():
        ...
        overall_df.to_csv(
            output_dir / f"conformal_overall_alpha_{alpha}.csv", index=False
        )
