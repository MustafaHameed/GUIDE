...
class ExplainabilityAnalyzer:
    ...
    def local_explanations(
        self, instance_indices: List[int], method: str = "shap", top_k: int = 10
    ) -> Dict[int, Dict]:
        ...
        """
        logger.info(
            f"Generating local explanations for {len(instance_indices)} instances using {method}"
        )
            ...
            try:
                if method == "shap" and self.shap_values is not None:
                    ...
                    if idx < len(self.shap_values):
                        ...
                        top_indices = np.argsort(np.abs(feature_contributions))[
                            -top_k:
                        ][::-1]
                    ...
                    lime_exp = self.lime_explainer.explain_instance(
                        instance, self.model.predict_proba, num_features=top_k
                    )

                    explanations[idx] = {
                        "features": [
                            self.feature_names[f] for f, _ in lime_exp.as_list()
                        ],
    ...
    def faithfulness_check(
        self, top_k: int = 5, method: str = "remove"
    ) -> Dict[str, float]:
        ...
        for feature in top_features:
            if feature in X_perturbed.columns:
                if method == "remove":
                    ...
                    # Shuffle values
                    X_perturbed[feature] = np.random.permutation(
                        X_perturbed[feature].values
                    )
        ...
        logger.info(
            f"Faithfulness check: AUC drop = {results['auc_drop']:.3f}, "
    ...
    def stability_by_group(self, group_column: str = "sex") -> Dict[str, pd.DataFrame]:
        ...
        for group_value in aligned_sensitive.unique():
            ...
            if len(group_shap) > 0:
                ...
                stability_results[group_value] = importance_df.sort_values(
    ...
    def save_pdp_ice_plots(
        self, output_dir: Path, features: Optional[List[str]] = None
    ) -> None:
        ...
        for feature in features:
            try:
                ...
                PartialDependenceDisplay.from_estimator(
                    self.model, self.X_test, [feature], kind="both", ax=ax
                )
                fig.savefig(
                    output_dir / f"pdp_ice_{feature}.png", dpi=300, bbox_inches="tight"
                )
                plt.close(fig)
            except Exception as e:
    ...
    def save_lime_explanations(
        self, figures_dir: Path, reports_dir: Path, instance_indices: List[int]
    ) -> None:
        ...
        for idx in instance_indices:
            try:
                ...
                fig.savefig(
                    figures_dir / f"lime_{idx}.png", dpi=300, bbox_inches="tight"
                )
                plt.close(fig)

            except Exception as e:
                logger.warning(
                    f"Failed to save LIME explanation for instance {idx}: {e}"
                )
    ...
    def generate_report(self, report_dir: Path, usage: str) -> None:
        ...
        with open(report_path, "w") as f:
            ...
            f.write(
                "SHAP summary, PDP/ICE plots, and LIME figures are stored in `figures/`.\n"
            )
...
def main():
    ...
    parser.add_argument(
        "--max-samples", type=int, default=100, help="Max samples for explanation"
    )
    parser.add_argument(
        "--sensitive-attr", default="sex", help="Sensitive attribute for stability"
    )
    ...
    analyzer = ExplainabilityAnalyzer(
        model, X_train, X_test, y_train, y_test, sensitive_features=sens_test
    )
