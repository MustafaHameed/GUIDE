...
def chronological_split(df: pd.DataFrame, output_dir: Path) -> Dict[str, List[int]]:
    ...
    ordered_presentations = [
        p for p in presentation_order if p in available_presentations
    ]

    if len(ordered_presentations) < 3:
        logger.warning(
            f"Only {len(ordered_presentations)} presentations available. Using all for training."
        )
    ...
    train_ids = (
        df[df["code_presentation"].isin(train_presentations)]["id_student"]
        .unique()
        .tolist()
    )
    val_ids = (
        df[df["code_presentation"].isin(val_presentations)]["id_student"]
        .unique()
        .tolist()
    )
    test_ids = (
        df[df["code_presentation"].isin(test_presentations)]["id_student"]
        .unique()
        .tolist()
    )
...
def module_holdout_split(
    ...
    for test_module in modules:
        ...
        if len(other_students) > 0:
            ...
            other_df = df[df["id_student"].isin(other_students)].drop_duplicates(
                "id_student"
            )
            ...
            try:
                ...
                if stratify_cols:
                    ...
                    if len(stratify_cols) > 1:
                        stratify_values = stratify_values.apply(
                    ...
                    train_students, val_students = train_test_split(
                        other_students, test_size=0.2, random_state=42
                    )
            except ValueError:
        ...
        # Check for leakage
        check_leakage(
            train_students.tolist(), val_students.tolist(), test_students.tolist()
        )
...
def random_split(
    ...
    try:
        if stratify_cols:
            ...
            if len(stratify_cols) > 1:
                stratify_var = stratify_df.apply(
                    lambda x: "_".join(x.astype(str)), axis=1
                )
            else:
            ...
            train_stratify = stratify_var[
                student_df["id_student"].isin(train_val_students)
            ]
            ...
            logger.warning(
                "No stratification variables available, using simple random split"
            )
            # Simple random split
            train_val_students, test_students = train_test_split(
                student_df["id_student"].values, test_size=test_size, random_state=42
            )

            val_size_adjusted = val_size / (train_size + val_size)
            train_students, val_students = train_test_split(
                train_val_students, test_size=val_size_adjusted, random_state=42
            )
